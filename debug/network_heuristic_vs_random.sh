...
echo "========================================"
RUST_LOG=debug ./target/release/mtg server --network-debug --seed=$SEED 2>&1 | tee "$OUTPUT_DIR/server.log" &
SERVER_PID=$!
...
timeout 120 bash -c "RUST_LOG=info ./target/release/mtg connect --seed-player=${SEED} --controller heuristic -n Ryan decks/booster_draft/avatar/ryan_avatar_draft.dck 2>&1" > "$OUTPUT_DIR/heuristic.log" &
P1_PID=$!
...
timeout 120 bash -c "RUST_LOG=info ./target/release/mtg connect --seed-player=${SEED} --controller random -n Gabriel decks/booster_draft/avatar/gabriel_avatar_draft.dck 2>&1" > "$OUTPUT_DIR/random.log" &
P2_PID=$!
