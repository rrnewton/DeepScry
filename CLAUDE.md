...
See `ai_docs/reference/ability_parsing_comparison.md` for detailed analysis and `ai_docs/reference/CARD_SCRIPT_SPEC.md` for the card script DSL specification.
...
When merging, archiving, or deleting a feature branch, also update `ai_docs/reference/OLD_BRANCH_HISTORY.md` with a brief note describing what the branch contained and its disposition (merged into integration/main, archived as tag, deleted, etc.). This preserves institutional memory about historical work even after the branch ref is gone.
