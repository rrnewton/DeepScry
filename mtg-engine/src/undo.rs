...
//! # Feature Flag: `undo`
//!
//! When the `undo` feature is enabled (default), this module provides full
//! undo/redo functionality for game tree search via rewind.
//!
//! When the `undo` feature is disabled:
//! - `UndoLog::log()` is a no-op with zero overhead
//! - `UndoLog::pop()` always returns `None`
//! - `game.undo()` returns an error
//! - Rewind-based benchmarks and tests are disabled
//!
//! Use `--no-default-features` to disable undo for snapshot-only replay benchmarks.
...
///
/// When the `undo` feature is disabled, the struct still exists for serialization
/// compatibility, but all mutating methods become no-ops.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UndoLog {
...
impl UndoLog {
    ...
    ///
    /// When the `undo` feature is enabled, pre-allocates capacity for typical game length.
    /// When disabled, creates a zero-size struct.
    #[cfg(feature = "undo")]
    pub fn new() -> Self {
    ...
    ///
    /// When `undo` feature is disabled, this is identical to `new()`.
    #[cfg(feature = "undo")]
    ...
    ///
    /// When `undo` feature is disabled, this is a no-op.
    #[cfg(feature = "undo")]
    #[inline]
    ...
    #[cfg(not(feature = "undo"))]
    #[inline(always)]
    pub fn log(&mut self, _action: GameAction, _prior_log_size: usize) {
        // No-op when undo feature is disabled
    }
    ...
    ///
    /// When `undo` feature is disabled, always returns `None`.
    #[cfg(feature = "undo")]
    ...
    ///
    /// Returns None if no ChangeTurn action is found in the log, or if undo is disabled.
    #[cfg(feature = "undo")]
    ...
    /// Rewind to turn start (stub when undo feature disabled)
    ///
    /// Always returns None when the undo feature is disabled.
    #[cfg(not(feature = "undo"))]
    pub fn rewind_to_turn_start<T>(&mut self, _game: &mut T) -> Option<(u32, Vec<GameAction>, usize)> {
        None
    }
