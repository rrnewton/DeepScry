...
use crate::game::log_event::{EventLogView, LogEvent};
...
pub struct GameLogger {
    ...
    /// Structured event log — the log-of-record for puzzle assertions.
    ///
    /// When `enable_event_log` is false (default) this stays empty and
    /// `push_event` is a no-op — zero overhead on MCTS/fuzz paths.
    /// Enable via [`Self::enable_event_log`] for puzzle runs and replays.
    ///
    /// NOT serialized: the custom Serialize impl skips it. On rewind callers
    /// use `truncate_events_to` to roll it back alongside the string buffer.
    event_log: Vec<LogEvent>,

    /// Whether structured event recording is active.
...
impl GameLogger {
    ...
    /// Enable structured event recording.
    ///
    /// Must be called before the game starts for the event log to be populated.
    /// By default event logging is OFF for zero overhead on MCTS/fuzz paths.
    /// Enable for puzzle runs, replays, or any path that needs event assertions.
    pub fn enable_event_log(&mut self) {
        self.event_log_enabled = true;
    }

    /// Disable structured event recording and clear the event log.
    pub fn disable_event_log(&mut self) {
        self.event_log_enabled = false;
        self.event_log.clear();
    }

    /// Whether structured event recording is currently enabled.
    #[inline]
    pub fn is_event_log_enabled(&self) -> bool {
        self.event_log_enabled
    }

    /// Record a structured game event.
    ///
    /// No-op (zero cost) when event logging is disabled.
    #[inline]
    ...
    /// Get the current number of structured events recorded.
    pub fn event_count(&self) -> usize {
        self.event_log.len()
    }

    /// Get a read-only view over the structured event log.
    ...
    /// Truncate the structured event log to a specific size.
    ///
    /// Used during undo/rewind to roll back events generated by undone actions,
    /// in the same way as [`Self::truncate_to`] for the string buffer.
    pub fn truncate_events_to(&mut self, size: usize) {
        if size < self.event_log.len() {
            self.event_log.truncate(size);
        }
    }

    /// Clear the structured event log (called alongside clear_logs on full resets).
...
impl Clone for GameLogger {
    fn clone(&self) -> Self {
        GameLogger {
            ...
            // Event log: clone starts empty, event_log_enabled inherits
            // from source so the clone can record events if needed.
            event_log: Vec::new(),
...
impl<'de> Deserialize<'de> for GameLogger {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Ok(GameLogger {
            ...
            suppressed: false,
            // Event log is never serialized; always starts fresh on deserialize.
            event_log: Vec::new(),
            event_log_enabled: false,
        })
    }
