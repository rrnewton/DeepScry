...
updated_at: 2026-05-28T13:30:46.408976416+00:00
---
...
Once the web-server unification lands (**mtg-468**, **mtg-469**, **mtg-470**), we need actual capacity numbers for the single-VM DeepScry deployment before we know how aggressively to advertise public play, whether to add a queue/backpressure layer, or whether to scale horizontally.
...
- Depends on **mtg-468** (unified server is what we're benchmarking — testing the dual-process setup would be wasted effort).
- Soft-depends on **mtg-470** (systemd's `LimitNOFILE`/`MemoryMax` may be the binding constraint at high N; useful but not required to start the benchmark).
