...
updated_at: 2026-05-29T18:26:50.162839121+00:00
---
...
- [x] Parses as {R} Instant; SP$ Charm with two modes (DBCounter / DBDestroy)
      offered as a mode choice.
- [BROKEN] The chosen mode's ValidTgts$ Card.Blue / Permanent.Blue target
  restriction is NOT enforced. REBL's 'Destroy target blue permanent' mode
  was allowed to target and destroy the caster's own RED Mountain — a
  non-blue permanent. Illegal target accepted (CR 115.4 / 608.2b).
  Filed as: mtg-af24s (Charm modes ignore ValidTgts).

CARD STATUS: BROKEN — modal Charm target restriction not enforced; can hit illegal (non-blue) targets. Blocked on mtg-af24s.
