...
updated_at: 2026-05-29T17:14:38.471948788+00:00
closed_at: 2026-05-29T17:14:38.471948645+00:00
---
...
1. [x] Parses as {0} Artifact.
2. [WORKING] A:AB$ Mana | Cost$ T Sac<1/CARDNAME> | Produced$ Any | Amount$ 3 —
   a mana ability with a tap + self-sacrifice cost that produces three mana of
   one chosen color. Verified funding a {1}{B}{B} cast with Black Lotus going to
   the graveyard from the sacrifice. CR 605, CR 701.16 (Sacrifice).

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/black_lotus_sac_mana.pzl \
  --p1=fixed --p2=zero --p1-fixed-inputs='cast Hypnotic Specter;pass;pass' \
...
```
  Tap Black Lotus for {B}{B}{B}
  Black Lotus (4) goes to graveyard
  Hypnotic Specter (3) enters the battlefield as a 2/2 creature
```

Unit test: test_card_compat_sol_ring_and_black_lotus in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/black_lotus_sac_mana_e2e.sh

CARD STATUS: WORKING — tap+sacrifice for three of one color verified.
