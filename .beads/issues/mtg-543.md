...
updated_at: 2026-05-29T17:14:38.470604122+00:00
closed_at: 2026-05-29T17:14:38.470603970+00:00
---
...
2. [WORKING] A:AB$ Mana | Cost$ T | Produced$ C | Amount$ 2 — a mana ability
   that taps for {C}{C}. Offered while paying for a spell (not as a standalone
   priority action, per CR 605 mana abilities). Verified funding a {4} cast.

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/sol_ring_mana.pzl \
  --p1=fixed --p2=zero --p1-fixed-inputs='cast Icy Manipulator;pass;pass' \
...
```
  Tap Sol Ring for mana
  Icy Manipulator (3) resolves
```

Unit test: test_card_compat_sol_ring_and_black_lotus in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/sol_ring_mana_e2e.sh

CARD STATUS: WORKING — {C}{C} mana ability verified.
