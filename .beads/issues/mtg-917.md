...
B4 [BROKEN] Island Sanctuary
  ...
  Impact: Island Sanctuary on the battlefield does nothing — no draw-skip offer, no protection granted.
  Fix shape: add R:Event$ Draw replacement handling in parse_static_abilities: parse Optional$ True + PlayerTurn$ True + ValidPlayer$ You + ReplaceWith$ <SVar>; at draw_step time, offer the active player the choice to skip their draw; if they do, create a persistent "can only be attacked by flying/islandwalk" effect for the turn. Non-trivial (new draw replacement event bus + per-turn persistent attack restriction targeting a specific player). Engine gap.
