...
updated_at: 2026-05-29T17:14:38.467957900+00:00
closed_at: 2026-05-29T17:14:38.467957747+00:00
---
...
1. [x] Parses as {1}{B} Sorcery.
2. [WORKING] A:SP$ ChangeZone | Origin$ Library | Destination$ Hand |
   ChangeNum$ 1 | Mandatory$ True — converts to a SearchLibrary effect; finds a
   card, puts it into hand, and shuffles the library. CR 701.19 (Search).

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/demonic_tutor_search.pzl \
  --p1=fixed --p2=zero --p1-fixed-inputs='cast Demonic Tutor;0;pass;pass' \
  --p2-fixed-inputs='' --stop-on-choice=5 --seed 42 --verbosity 3
```

Expected log evidence:

```
  Library search: found Black Lotus
  Demonic Tutor (3) searches Player 1's library for a Card card and puts it into Hand
```

Unit test: test_card_compat_demonic_tutor in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/demonic_tutor_search_e2e.sh

CARD STATUS: WORKING — library search to hand + shuffle verified.
