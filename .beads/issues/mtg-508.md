...
updated_at: 2026-05-29T17:14:38.469328803+00:00
closed_at: 2026-05-29T17:14:38.469328650+00:00
---
...
1. [x] Parses as {3}{B} Enchantment.
2. [WORKING] A:AB$ Draw | Cost$ B PayLife<2> | NumCards$ 1 — activating pays
   {B} and 2 life (Cost::PayLife deducts life via the cost-payment path) and
   draws a card. CR 602 (activated abilities), CR 119 (life payment). Minor log
   gap: the 2-life payment is deducted but not separately logged.

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/greed_draw.pzl \
  --p1=fixed --p2=zero --p1-fixed-inputs='activate Greed;pass;pass' \
  --p2-fixed-inputs='' --stop-on-choice=2 --seed 42 --verbosity 3
```

Expected log evidence:

```
  Greed activates ability: Draw a card.
  Player 1 draws Swamp (9)
```

Unit test: test_card_compat_greed in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/greed_draw_e2e.sh

CARD STATUS: WORKING — {B}+2life draw activation verified.
