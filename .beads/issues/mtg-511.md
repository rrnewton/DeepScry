...
updated_at: 2026-05-29T16:33:16.625443731+00:00
---
...
2. [PARTIAL] A:AB$ Tap | Cost$ 1 T | ValidTgts$ Artifact,Creature,Land — the
   ...
   auto-targets the SOURCE (Icy Manipulator itself) rather than an opponent
   permanent, with both fixed and heuristic controllers. The ability resolves
   but taps itself, so the AI can't use it usefully.
   Filed as: Bug: AB$ Tap auto-targets its own source (mtg-zssaf).

CARD STATUS: PARTIAL — mana/tap mechanics work; activated-ability target
selection picks the source instead of an opponent permanent (mtg-zssaf).
