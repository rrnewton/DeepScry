...
updated_at: 2026-05-31T16:22:03.404992218+00:00
---
...
1. [x] Parses as {2} Artifact and casts/resolves.
2. [BROKEN] The ARN-sacrifice trigger never fires and the cast/play
   prohibition is not enforced. Three unsupported constructs:
   - setARN set-origin matching (Permanent.setARN / Card.setARN)
   - T:Mode$ Always state-trigger
   - S:Mode$ CantBeCast / CantPlayLand statics
   Verified: Library of Alexandria (an ARN card) survives City in a Bottle.
   Filed as: mtg-3hwz3 (setARN + Always-trigger + cast-prohibition gap).

Reproducer:

```sh
./target/release/mtg tui --start-state /tmp/bottle.pzl \
  --p1-fixed-inputs='cast City in a Bottle' --p1=fixed --p2=zero \
  --seed 42 --verbosity 3 --stop-when-fixed-exhausted
## (p1 bf = Library of Alexandria; expect it to be sacrificed — it is NOT)
```

CARD STATUS: BROKEN — setARN matching, Always-trigger, and cast-prohibition
statics unimplemented. Blocked on mtg-3hwz3. Niche sideboard hoser; does not
block robots-deck playability.

== RESOLVED 2026-05-31_#2549(221c7867), compat-finish-robots ==
CARD STATUS: WORKING. Engine gap mtg-3hwz3 is CLOSED (@6c7d384e) — all three
constructs implemented generally: (1) set-origin filter (Card.setARN via
serialized origin_set), (2) Mode$ Always state-trigger as a deterministic SBA-like
sweep (SacrificeAll of matching permanents), (3) CantBeCast / CantPlayLand statics
(prohibited cards never offered as legal plays). Verified at this SHA:
- Unit: test_card_compat_city_in_a_bottle PASS (all 3 statics parse w/ setARN/!token/Other).
- E2E: test_city_in_a_bottle_arn_hoser (puzzle_e2e.rs) PASS.
- Reproducer log (mtg tui --start-state test_puzzles/city_in_a_bottle_arn_hoser.pzl
  --p1=zero --p2=zero --seed 42): "Camel (5) is sacrificed (originally-printed-set
  hoser)" while a non-ARN Grizzly Bears survives and attacks; the held ARN Camel
  in hand is never castable.
Rules-review PASS recorded in mtg-3hwz3 (Mode$ Always as deterministic sweep,
CR 605). Closing.
