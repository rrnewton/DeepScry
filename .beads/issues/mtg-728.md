...
updated_at: 2026-06-06T00:50:04.858185421+00:00
---

# Description

>## REVERTED 2026-06-04 (slot02) — action_count exclusion removed; principled successor landed (mtg-752)

The interim action_count exclusion from compute_view_hash (this issue's class-A
workaround, mtg-728 / mtg-744) has been REVERTED. action_count is once again
...
consensus-undo-log unification (mtg-752) removed that drift at the source
...
canary + make validate green with action_count re-included (see mtg-752). The
...
known-red pending reliability confirmation (mtg-774).
...
seed-2 blocker is mtg-744 (network-layer hash-validation pairing / shadow
...
CONCLUSION: seed-2 == mtg-744 (UPGRADED there to confirmed real+blocking, with
...
remain GREEN regression guards; R2 ruled out (mtg-745). Escalated to coordinator
...
------------------------------------------------------------------------
mtg-733 (mana_pool per-action undo gap) FIXED (see mtg-733): SetManaPool{prev} undo action + log_mana_pool helper before all 5 pay sites + partial-undo test.
...
ROOT-CAUSE CLASS (mtg-728 + mtg-725): the WASM shadow records None for opponent
...
GREEN => done; matching ac but still desync => the seq/hash misalignment (mtg-744) is
...
FOLLOW-UP FILED: mtg-744 (seq↔ac↔hash desync-detection misalignment; resolves post-fix
...
mtg-745 — do NOT count-parity-hack it). Browser seq/ac/hash desync-detection
misalignment filed mtg-744 (resolves post-fix; do not chase now). Enumeration +
...
gap->0 + still-red = mtg-744 misalignment is the residual (escalate).
