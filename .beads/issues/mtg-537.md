...
updated_at: 2026-06-14T10:09:52.421099026+00:00
closed_at: 2026-05-29T17:14:38.462316460+00:00
---
...
1. [x] Parses as {1}{B}{B} 1/1 Creature - Human Assassin.
2. [WORKING] A:AB$ Destroy | Cost$ T | ValidTgts$ Creature.tapped — Royal
   Assassin taps to destroy a TAPPED creature; the destroyed creature goes to
   the graveyard. The Creature.tapped target restriction is honored: against an
   UNTAPPED creature the ability is not offered at all (verified separately).
   CR 701.7 (Destroy), CR 602 (activated abilities).

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/royal_assassin_destroys_tapped.pzl \
  --p1=fixed --p2=zero --p1-fixed-inputs='activate Royal Assassin;0;pass;pass;pass' \
  --p2-fixed-inputs='' --stop-on-choice=6 --seed 42 --verbosity 3
```
...
```
  Royal Assassin activates ability: Destroy target tapped creature.
    -> targeting Serra Angel (10)
  Serra Angel (10) goes to graveyard
```

Unit test: test_card_compat_royal_assassin in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/royal_assassin_destroys_tapped_e2e.sh

CARD STATUS: WORKING — tap-to-destroy-tapped-creature verified.

== Puzzle-backed (card-audit wave 2, mtg-948 Part B, 2026-06-14_#3463(2a27999fe)) ==

PUZZLE_FILE: test_puzzles/audit_royal_assassin_destroys_tapped.pzl

Scripted ACTIVATED ability: P0 activates Royal Assassin ({T}: destroy target tapped creature) against P1's TAPPED Serra Angel -> Serra Angel to P1 graveyard; Royal Assassin survives (only tapped). AB$ Destroy Cost$ T ValidTgts$ Creature.tapped; CR 602/701.7.
