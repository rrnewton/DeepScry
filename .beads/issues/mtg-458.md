...
updated_at: 2026-05-28T13:30:42.778283514+00:00
---
...
After fixing the test-harness wait-loop hang (mtg-457), the `cycle_ability_network_sync_e2e.sh` regression test now runs to completion (no more 180s timeout) but FAILS its core assertion: LOCAL and SERVER gamelogs diverge by ~232 lines for seed=315 random/random.
...
The wait-loop fix (mtg-457) exposed this because before it, the test was timing out at 180s before reaching the gamelog comparison, so nobody noticed the seed mismatch the centralization commit introduced.
...
- mtg-457 (lobby-lifecycle wait-loop fix that exposed this)
- mtg-420 (original cycle/Mountaincycling desync fix; still valid, just was being tested with mismatched seeds)
- mtg-430 (separate snapshot bincode regression on same HEAD)
