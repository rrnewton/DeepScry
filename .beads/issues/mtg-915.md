...
Root cause class: missing ApiType::RepeatEach + the per-element repeat-subability loop (iterate DefinedCards, bind Remembered, run sub-ability). SAME gap as 2005 backlog (mtg-911 referenced RepeatEach-adjacent) — coordinate one fix. Affects Terastodon, Avenger-of-Zendikar-style 'each' payoffs that route through RepeatEach, etc.
...
== WAVE-1 ADDITIONAL FIX (2026-06-12_#3274, slot28/compat-2010-wave1) ==
B-token [FIXED] TokenAmount$ Count expression — Avenger of Zendikar creates N tokens per land:
Root cause: create_token_dynamic_from_params (loader/card.rs) returned None for DynamicAmount::Count,
falling through to params_to_effect which defaulted TokenAmount=1. Fix: route Count through to
CreateTokenDynamic; evaluate_count_expression at execute_effect time. Also: resolve controller
placeholder in triggers.rs resolve_effect_placeholder for CreateTokenDynamic{Count}.
Affects 957 cards with TokenAmount$ X/Y/N SVar references. CR 303.4b / CR 608.2.
Regression test: test_token_amount_count_expression_avenger_of_zendikar (PASSES).

== WAVE-3 FIX (2026-06-12_#3302(829218b5c), slot42/compat-2010-wave3) ==
B7 (unlisted) [FIXED] withDefender qualifier — Overgrown Battlement + ~35 card-script lines using Creature.withDefender:
Root cause: `count_permanents_matching()` had no logic for `withDefender` filter, always returning 0;
`TargetRestriction::parse()` likewise never set any Defender-keyword check. Fix: (1) add
`requires_defender: bool` field (#[serde(default)]) to TargetRestriction struct, set in parse(),
checked via `card.has_keyword(Keyword::Defender)` in matches(). (2) add `withDefender` branch in
count_permanents_matching after the type check. Affects Overgrown Battlement (deck04), Axebane
Guardian, Carven Caryatid, Blight Pile, and ~32 other card scripts. Test:
test_withdefender_filter_count_and_target_restriction (parse sets flag, matches enforces it, count
returns 2 of 3 creatures). Validate PASS: validate_logs/validate_829218b5c248761e0bf77c70fdcd40155795c782.log.
CR 702.6a: "A creature with defender can't attack."

== Cross-links ==
Tracker mtg-914. Umbrella mtg-684. Sibling backlogs: mtg-713 (1994), mtg-902 (2020), mtg-v59ll (2025), mtg-911 (2005). Shared root-cause gaps with 2005: RepeatEach (B2 ~ 2005), NameCard (B4 ~ 2005 B5) — fix once, fix both. Survey artifacts (gitignored): debug/2010survey/{tourney.log, v3_ub_vs_eldrazi.log, v3_ub_mirror.log, v3_eldrazi_mirror.log, unique_cards.txt}.
