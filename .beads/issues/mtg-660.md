...
updated_at: 2026-06-01T15:39:26.497234346+00:00
---
...
Consequences:
1. The per-resolution non-combat damage accumulator (GameState::damage_dealt_by_source, added in mtg-647) is NOT fed, so a 'whenever ~ deals damage' trigger (Spirit Link CR 119.3) would not fire if an enchanted creature were ever the SOURCE of a DamageAll. In practice DamageAll's source is the resolving spell (not the enchanted creature), so Spirit Link is unaffected today -- hence out of scope for mtg-647.
