...
updated_at: 2026-05-29T17:14:38.460889388+00:00
closed_at: 2026-05-29T17:14:38.460889246+00:00
---
...
1. [x] Parses as {B}{B}{B} Enchantment.
2. [WORKING] T:Mode$ Drawn | ValidCard$ Card.OppOwn | Execute$ TrigDamage
   (DB$ DealDamage | Defined$ TriggeredPlayer | NumDmg$ 1) — when an OPPONENT
   draws a card, Underworld Dreams deals 1 damage to that drawing player. The
   controller's own draws do NOT trigger it (verified: P0 drew with no damage,
   then P1 drew and went 20->19). CR 603 (triggered abilities).

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/underworld_dreams_draw_damage.pzl \
  --p1=zero --p2=fixed --p2-fixed-inputs='' --stop-on-choice=3 --seed 42 --verbosity 3
```

Expected log evidence:

```
  Player 2 draws Plains (20)
  Trigger: Underworld Dreams - Whenever an opponent draws a card, CARDNAME deals 1 damage to that player.
  Life: 19
```

Unit test: test_card_compat_underworld_dreams in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/underworld_dreams_draw_damage_e2e.sh

CARD STATUS: WORKING — opponent-draw damage trigger verified.
