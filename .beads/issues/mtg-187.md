...
updated_at: 2026-05-31T19:03:41.262354177+00:00
---
...
## Phase 2 (2026-05-31_#2560)

Phase 2 complete (branch phase2-native-network, commit d80db0bc):

### Deliverable 1 — Native GUI network client
- native_game.html now has a real network client (mirrors tui_game.html).
  Game-mode select (Local / Custom Network Game), server-url/password/player-name
  fields, and updateGameModeUI() all added.
- Network path uses launch_network_game + TUI renderer when a lobby game starts.
- window.__mtgNetworkClient is now live when network mode is active.

### Deliverable 2 — Shared launcher (web/lobby_launcher.js)
- New ES module: consumeLobbyParams(), applyLobbyParamsToForm(), buildLobbyAction(), buildRedirectUrl().
- Both tui_game.html and native_game.html import and use the shared module.
- maybeAutoLaunchFromLobby() in tui_game.html simplified to ~10 lines.
- native_game.html now auto-launches from ?lobby_create=/?lobby_join= params.
- index.html: 'Game Interface' TUI/Native radio picker added to Create form.
- lobby_launcher.js added to HASHED_JS_LEAVES in asset_hash.rs.
- test_web_server_smoke.js: lobby_launcher.js is hashed, fixed-name 404s, immutable.
- test_landing_page_ux.js: scenarioNativeGuiLaunch() verifies Native radio routes correctly.

### Deliverable 3 — Rename
- 'Remote Network Game' → 'Custom Network Game' in tui_game.html and native_game.html.

### Validate result: PASS (1430/1430 tests, DIVERGED: 0, Failed: 0)
Log: validate_logs/validate_d80db0bc7fc40efbacb4dac86fa1f2e708fb4fb1.log
