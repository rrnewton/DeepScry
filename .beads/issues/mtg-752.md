...
updated_at: 2026-06-06T00:50:04.891138209+00:00
---
...
EMPIRICAL PROOF (probe @6a88ead1 + recipe, mtime-fresh, isolated systemd-run scope):
 - full un-excluded desync canary RESULT: PASS — avatar-cycling 18/18, monored 18/18,
   ...
   known-red, see mtg-774). ZERO new red, ZERO nondeterminism.
...
"completely finished": the action_count exclusion (mtg-728) is reverted, its
...
VALIDATE RE-ENABLE @25cb0454 (scripts/validate.py): removed the mtg-769
...
PRE-EXISTING avatar/cycling mirror nondeterminism (mtg-725 / mtg-768 class), NOT a
...
7. Bug-class generalization: class-level — fixes ALL shuffle-then-draw cards
   ...
   class (mtg-725 / mtg-768), unaffected by this change.
...
in parent too and tracked under mtg-725 / mtg-768, explicitly out of scope.
...
mtg-768 asks (team-lead): (1) call_pre_choice_hook now applies state-sync up to
...
mtg-609), NOT subsumed by the buffer shim. Tracked in mtg-768; not chased.
...
7. Bug-class generalization: class-level (all reveal/reorder/opponent-choice
   ...
   tracked in mtg-768 (pre-existing, not introduced here).
...
deeper divergence already tracked in mtg-768 and explicitly out of scope.
...
GATE PROGRESS:
- NATIVE un-excluded GREEN (acceptance prize): netarch_lockstep_oracle_e2e full 13-seed sweep PASSES with action_count RE-INCLUDED in compute_network_state_hash — CLASS_A [1,2,5,6,7,9,11,18,19,20] (incl mtg-744 2/5/1/6) + CONTROL [3,13,16] (Hallows-3). native_wasm_equiv_sweep 15/0 DIVERGED.
...
- Searched-dummy STAYS at the search-resolution ac (load-bearing for searched_card_for; never re-stamp earlier → mtg-728 regress). RED test pins it.
...
Native-first pivot landed the WASM-disable (@b67d8428, mtg-769). Characterizing the native baseline revealed "Blocker A" (equiv-zero seed3) is NOT a deterministic cycling/buffer desync:
...
mtg-752 "reveal not aligned at the correct action_count" class,
...
adversarial desync-review + MTG-rules review pending. Keeps hash ID mtg-752.
...
CHANGES (committed on fix-deep-ac, prize still OFF):
 1. mtg-795 [PRIMARY]: actions/mod.rs execute_balance_effect Hand-zone discard
    ...
    seed11 idx 382).
 2. mtg-797: undo.rs reconstruct_controller_states() + client.rs applies it on
    re-materialization (twin of reconstruct_tapped_states; restores hashed
    'controller'). Robots can't exercise it; unit-tested.
 3. mtg-789 #1: protocol.rs SubmitChoice.spell_ability stale doc corrected.
...
STILL BLOCKING THE PRIZE (checkpointed):
 - seed 7: P1 hand-size off-by-one (server 6 vs client 5) at choice_seq=230
   ...
   fix = reveal-actionlog unification (mtg-799).
 - seed 19 (mtg-796): client sends illegal choice index 2 (server offers 2) at
   WebRandom Fireball cast turn 24 — option-set/state divergence (mtg-784 family).
 - mtg-789 #2 (WASM spell_ability populate): deferred (crash-earlier guard).
...
(ac -2). Deep-ac in-stack-resolution reveal-application LAG (mtg-752/mtg-559 family),
NOT the tapped/controller re-materialization class. Principled fix = reveal-actionlog
unification (mtg-799): drive search-to-hand through the action_count-keyed consensus
...
choice-hash-timing half of mtg-799 (make both replicas hash a given choice_seq at
the same ac), NOT a missing-delta. Full: ai_docs/transient/DEEPAC_BLOCKERS_CHECKPOINT_20260605.md.

## SEED-7 DECISIVE RE-DIAGNOSIS 2026-06-05 (slot03-lockstep) — corrects prior 'stamping skew' reframe; this is missing-opponent-delta (mtg-799 §1-2)
