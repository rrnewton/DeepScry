...
The per-source toggle checkboxes (#img-src-local, #img-src-scryfall, #img-src-gatherer) in game.html are read into the settings object but never actually consulted when building the fallback list. Only the master "Show Card Images" checkbox gates rendering.
