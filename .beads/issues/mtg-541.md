...
updated_at: 2026-05-29T14:38:37.072099003+00:00
---
...
Shivan Dragon: {4}{R}{R} 5/5 Dragon, K:Flying, A:AB$ Pump | Cost$ R |
NumAtt$ +1 (firebreathing). No engine change needed — verification-to-WORKING.

- [WORKING] Parses as 5/5 {4}{R}{R} Creature Dragon with Flying.
- [WORKING] Firebreathing: {R}: +1/+0 activates repeatedly and the buff
  expires at end of turn (5/5 -> 10/5 in combat, back to 5/5 next turn).
  Covered by puzzle_e2e.rs test_shivan_dragon_firebreathing_combat /
  test_shivan_dragon_pump_ability.
- [WORKING] Flying: a non-flying / non-reach creature (Grizzly Bears) may
  NOT block it (CR 509.1b / 702.9b); Shivan's damage hits the player.

Expected log evidence:
  Player 1 declares Shivan Dragon (N) (7/5) as attacker
  Shivan Dragon (N) deals 7 damage to Player 2
  Shivan Dragon activates ability: Shivan Dragon gets +1/+0 until end of turn.

Reproducer (flying block):

```sh
./target/release/mtg tui --start-state test_puzzles/shivan_dragon_flying_block.pzl \
  --p1=heuristic --p2=heuristic --seed 5 --verbosity 2
```
Expect in stdout: `declares Shivan Dragon ... as attacker`, NO
`declares Grizzly Bears ... as blocker for Shivan Dragon`, and
`Shivan Dragon (N) deals N damage to Player 2`.

Unit test: test_card_compat_shivan_dragon in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/shivan_dragon_flying_block_e2e.sh (flying) +
           puzzle_e2e.rs firebreathing/pump tests

CARD STATUS: WORKING — Flying + firebreathing both correct.
