...
ai_docs/transient/DEEPAC_SEED5_ROOT_PIN_20260605.md (this branch).
...
Full diagnosis: ai_docs/transient/DEEPAC_BLOCKERS_CHECKPOINT_20260605.md. Wheel-of-Fortune
...
the same ac), NOT a missing-delta. Full: ai_docs/transient/DEEPAC_BLOCKERS_CHECKPOINT_20260605.md.
...
## SEED-7 FIXED 2026-06-05 (slot03-lockstep) — partial-valid_cards index for Remote opponent search
Seed 7 now PASSES; all 8 previously-green seeds (2,5,6,9,11,18,20,42) still PASS (no regression); seed 19 still fails at its known Mishra-Factory-animation point. The actions/mod.rs:4777 deeper-pin was a misread (probe placed after the !is_network_mode early-return). Entry probe showed: choose_from_library_with_hook IS the path (its !is_network_mode branch; legacy hook unconfigured so is_network_mode=false). Bug: the OBSERVER's valid_cards is only the materialized subset of the opponent library (4 of server's 37); the match fell back to the authoritative CardId only when valid_cards.is_empty(), so a PARTIAL non-empty list took the index arm -> wrong card or (index>=len) None -> tutored card 97 lost. FIX (2 parts): (1) network_choice.rs choose_from_library_with_hook: for a Remote controller use the authoritative searched_card_lookup -> take_library_search_result, ignoring the partial index; (2) network/client.rs: add native SharedNetworkState::searched_card_for + wire with_searched_card_lookup on the native GameLoop (was WASM-only) for rewind survival. +73 lines, 2 files. fmt+clippy+clippy-wasm green. Full writeup: ai_docs/transient/DEEPAC_SEED7_MEMBERSHIP_CONFIRM_20260605.md (FIXED section). NOT merged: team-lead desync-review + all-10-strict re-verify pending. mtg-ho2r8 can move toward closed for the search-to-hand class once verified.
