...
SUPERSEDES the cwd-scoped /proc-reaper idea (was folded into mtg-726): cgroup-scoping is strictly more robust (captures setsid escapees + atomic kill, no /proc race). Likely CLOSES mtg-737 + the All-Hallow's-Eve H2 port-collision class outright. Also subsumes slot03's Playwright-screenshot-flake mitigation context (the contention these zombies cause).
...
(netns) are the mtg-743 follow-on design — do NOT balloon the 12/12 path.
