...
updated_at: 2026-05-29T14:30:31.684203091+00:00
---
...
Scrubland is a NO-printed-mana-ability dual land (Types: Land Plains Swamp).
Both colours come from intrinsic "{T}: Add {color}" abilities granted by the
two basic land subtypes per CR 305.6 (loader/card.rs:500-565). NO engine change
needed — verification-to-WORKING.

- [WORKING] Parses as Land with subtypes Plains + Swamp; no Effect::Unimplemented.
- [WORKING] Grants exactly two intrinsic {T} mana abilities: {W} and {B}.
- [WORKING] Mana from this card is spendable: in the Sengir reproducer the
  heuristic taps Scrubland for {W} as part of paying for spells.
...
```sh
cat > /tmp/dual_lands.pzl <<'P'
[metadata]
Name=Dual Lands Mana Test
Goal=Win
Turns=3
[state]
turn=4
...
activephase=MAIN1
p0life=20
p0hand=Sengir Vampire
p0battlefield=Badlands;Scrubland;Bayou;Swamp;Swamp
p0library=Swamp;Swamp;Swamp
p1life=20
p1library=Swamp;Swamp;Swamp
P
./target/release/mtg tui --start-state /tmp/dual_lands.pzl --p1 heuristic --p2 zero --seed 42 -v 3
```
Expect in stdout: `Tap Scrubland for {W}`.

The {W}/{B} ability shape is pinned by the unit test below (which asserts
each dual land grants exactly its two correct colours).

Unit test: test_card_compat_original_dual_lands in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/dual_lands_mana_e2e.sh (covers Badlands {R} + Bayou {G} sides)

CARD STATUS: WORKING — both intrinsic mana colours ({W}/{B}) produced and spendable.
