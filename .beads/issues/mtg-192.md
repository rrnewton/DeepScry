...
updated_at: 2026-06-01T13:44:06.987825511+00:00
---
...
---
GARDENING (2026-06-01): partially-stale, needs re-check. The issue was last updated 2026-01-03. Since then:
- Balance (SP$ Balance API type) was likely fixed as part of old-school deck work (Balance is WORKING per mtg-483 CLOSED)
- White Weenie deck is in the network tests (web/test_network_multideck.js references white_weenie seed=7 for desync tracking)

Open pieces per the Summary:
1. Land Tax trigger conditions (CheckSVar$/SVarCompare$ eval for phase triggers) — mtg-111
2. Land Tax tutor effect (DB$ ChangeZone in phase trigger) — mtg-111
3. Balance spell effect (SP$ Balance API) — LIKELY DONE (mtg-483 closed)

GARDENING: re-check whether Balance is truly fixed and whether the deck plays end-to-end.
