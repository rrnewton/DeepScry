...
updated_at: 2026-06-06T00:50:04.831014815+00:00
closed_at: 2026-06-04T08:46:50.725935097+00:00
---

# Description

PERF / ARCHITECTURE follow-up split out from mtg-750 (the flickering-opponent-card bug). 

CONTEXT: mtg-750 fixed the user-VISIBLE flicker with a resolved-URL image MEMO (native_game.html) — once a card image loads, the working URL is served first on later renders so the recreated <img> paints from cache (no re-cascade, no flicker). That is the SURGICAL symptom fix and is shipped.
...
WHY SEPARATE: the memo makes the flicker impossible even with the thrash, so this is a perf/architecture cleanup, not a correctness blocker. Keeping it out of mtg-750 keeps that bug fix tight + bankable.

RELATED: mtg-748 (battlefield layout-engine first-principles review — flagged render oscillation / the Rust->GUI dataflow), mtg-m9znz (the prior same-class card-details tick fix), mtg-750 (the flicker bug whose memo this complements).

── MEASUREMENT + RE-SCOPE 2026-06-04 (slot04, fix-mtg-751) ──
