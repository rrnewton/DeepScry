...
updated_at: 2026-05-29T19:34:45.705609797+00:00
---
...
## 2026-05-29: ATTEMPTED on branch ci-larger-runner-mtg599 — label NOT available on this account
Pushed branch with `runs-on: ubuntu-latest-4-cores` for test-determinism (line 129) + test-shell (line 207). Both jobs sat in `queued` for 12+ minutes with no runner pickup (typical pickup <30s), then I canceled the run. That's the standard signature of the larger-runner label not being enabled on the account/repo.
TO ENABLE: GitHub Settings → Actions → Runners → Larger runners (requires a paid plan: Pro/Team/Enterprise, or pay-per-minute for Pro). Once enabled, ff-merge branch `ci-larger-runner-mtg599` into integration.
Branch left in place (not merged) for the user to merge after enabling. Safe to revert/delete the branch if you don't want the change pending.
