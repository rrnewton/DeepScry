...
updated_at: 2026-06-01T23:17:05.007255070+00:00
---
...
This edits web/*.html heavily — SEQUENCE AFTER the branding/version web work (mtg-618/czueh/1iu9b) to avoid conflicts.

## UPDATE 2026-06-01 (cas-general-renamer, mtg-682): generalized into a graph-aware renamer + fixed the lobby-redo deploy break (bug mtg-dtgcn)
The mtg-620 hasher had two flaws (see mtg-dtgcn): hardcoded HASHED_HTML_PAGES (missed launcher.html) + redirect_cross_html_to_entry star-topology hack (flattened forward-nav). Reworked asset_hash.rs::asset_graph into a GENERAL renamer:
- AUTO-DISCOVER html (dir glob, sorted) — no hardcoded page list.
- Reference graph: MODULE edges (import/<script src>) = a DAG, hashed in topological order so forward-links resolve to hashed names statically.
- True cycle {tui_game.html, native_game.html, lobby_launcher.js} broken via a SERVED manifest (asset-manifest.json + stable web/asset_manifest.js loader) for the soft cross-refs; the one-way module edge (pages→lobby_launcher.js) resolves statically (launcher hashed first). NO entry-redirect hack.
- index.html stays the sole stable bootstrap; asset_manifest.js is the second stable name (the loader).
Gates: web/test_deploy_tree_nav.js (HASHED-tree nav gate, FAIL-on-old/PASS-on-new, wired into make validate), tests/asset_graph_hash.rs (auto-discovery + manifest), test_web_server_smoke.js (HTML immutable). mtg-620 can close once this lands.
