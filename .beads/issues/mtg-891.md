...
updated_at: 2026-06-10T19:07:36.322866182+00:00
---
...
VERIFIED LANDED 2026-06-10 (claude/web-card-render-robustness review): all four
web fixes — in-list AI-watch choices (computeMetaChoices/selectMetaChoice),
WS-disconnect red banner (__showNetworkError), debug-gated [Network] logs
(network.js _log/this.debug), and the Safari card-flicker node reuse
(renderBattlefield pools tiles by card_id; createCardElement/updateCardElement
reuse the <img> via data-img-key; decoding=sync on small tiles) — are present
on integration tip (re-landed after the temporary revert 8cc7dfac, via commits
c2123fac/67e601e7/f2e1e602/bed9f796/66bc207c). No further flicker work needed.
The companion test hardening (mtg-808) is now also resolved.
