...
updated_at: 2026-06-14T10:09:52.455305060+00:00
---
...
Disenchant: {1}{W} Instant, A:SP$ Destroy | ValidTgts$ Artifact,Enchantment.
No engine change needed — verification-to-WORKING.

- [WORKING] Parses as {1}{W} Instant with an SP$ Destroy spell ability
  targeting Artifact OR Enchantment (asserted via tokenized
  AbilityParams::parse — no substring matching).
- [WORKING] Casts at instant speed, targets the opponent's artifact, and
  destroys it; the artifact moves to its owner's graveyard.

Expected log evidence:
  Player 1 casts Disenchant (N) (putting on stack)
    -> targeting Jalum Tome (N)
  Jalum Tome (N) goes to graveyard
  Disenchant (N) destroys Jalum Tome (N)

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/disenchant_destroys_artifact.pzl \
  --p1=fixed --p2=zero \
  --p1-fixed-inputs="cast Disenchant;*;*" \
  --stop-on-choice=6 --seed 42 --verbosity 3
```
Expect in stdout: `Disenchant (N) destroys Jalum Tome` and
`Jalum Tome (N) goes to graveyard`.

Unit test: test_card_compat_disenchant in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/disenchant_destroys_artifact_e2e.sh

CARD STATUS: WORKING — destroys target artifact/enchantment.

== Puzzle-backed (card-audit wave 2, mtg-948 Part B, 2026-06-14_#3463(2a27999fe)) ==

PUZZLE_FILE: test_puzzles/audit_disenchant_destroys_artifact.pzl

Scripted: P0 casts Disenchant ({1}{W}) at P1's Mox Jet (vanilla artifact) -> destroyed to P1 graveyard. Artifact target restriction honored. SP$ Destroy ValidTgts$ Artifact,Enchantment; CR 701.7.
