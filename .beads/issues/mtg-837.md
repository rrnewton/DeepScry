...
updated_at: 2026-06-06T08:45:58.806420305+00:00
---
...
4. [PARTIAL] Noncreature SpellCast trigger: fires correctly but log says "+1/+1 counter" instead of "gains 1 loyalty" (mtg-823). State IS correct.
5. [BROKEN] Otter token Prowess fizzles (mtg-822) — Prowess on Otter tokens does not apply.
...
CARD STATUS: PARTIAL — casts/activates/triggers work; log bug for loyalty trigger (mtg-823); Prowess on tokens broken (mtg-822)

## Fix (2026-06-06):
- Prowess fizzle (mtg-822) FIXED — Otter tokens now correctly get +1/+1 from Prowess when noncreature spells are cast. Verified: tokens grow to 3/3 in test.
- SpellCast loyalty counter log fixed (mtg-823 shared fix).
