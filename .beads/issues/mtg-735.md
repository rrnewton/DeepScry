...
updated_at: 2026-06-06T00:50:04.859942427+00:00
---

# Description

Follow-up to mtg-713 B12 / mtg-734 (Kismet). The structured global ETB-tapped replacement (CardDefinition::etb_tapped_global) only installs predicates whose ValidCard$ qualifiers are controller restrictions we model (OppCtrl/YouCtrl/Any), with ActiveZones Battlefield and no IsPresent/ValidCause. Predicates with qualifiers TargetRestriction::parse silently DROPS are deliberately REFUSED (left no-op) to avoid over-matching (e.g. tapping basic lands / your own creatures).
