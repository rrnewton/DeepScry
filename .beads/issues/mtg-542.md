...
updated_at: 2026-06-14T11:24:10.130060094+00:00
closed_at: 2026-05-29T17:14:38.466538819+00:00
---
...
1. [x] Parses as {B}{B} Sorcery.
2. [WORKING] A:SP$ Destroy | ValidTgts$ Land — destroys a target land (moves it
   to the graveyard). CR 701.7 (Destroy). Note: 'Destroy target land' has no
   controller restriction, so own lands are also legal targets (matches the
   printed card); the test isolates the opponent's land as the only legal target.

Reproducer:

```sh
./target/release/mtg tui --start-state test_puzzles/sinkhole_destroys_land.pzl \
  --p1=fixed --p2=zero --p1-fixed-inputs='cast Sinkhole;pass;pass' \
...
```
  Sinkhole (3) destroys Plains (9)
  Plains (9) goes to graveyard
```

Unit test: test_card_compat_sinkhole in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/sinkhole_destroys_land_e2e.sh

CARD STATUS: WORKING — target-land destruction verified.

== AUDIT WAVE 3 (mtg-948 Part B), 2026-06-14 — PUZZLE-BACKED ==
PUZZLE_FILE: test_puzzles/audit_sinkhole_destroys_land.pzl
