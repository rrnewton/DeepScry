...
updated_at: 2026-05-29T14:30:31.675950978+00:00
---
...
Bayou is a NO-printed-mana-ability dual land (Types: Land Swamp Forest). Both
colours come from intrinsic "{T}: Add {color}" abilities granted by the two
basic land subtypes per CR 305.6 (loader/card.rs:500-565). NO engine change
needed — verification-to-WORKING.

- [WORKING] Parses as Land with subtypes Swamp + Forest; no Effect::Unimplemented.
- [WORKING] Grants exactly two intrinsic {T} mana abilities: {B} and {G}.
- [WORKING] Mana from this card pays for spells: taps for {B} to cast Sengir
  Vampire ({3}{B}{B}) and taps for {G} to cast Giant Growth ({G}).

Expected log evidence:
  Tap Bayou for {B}
  Tap Bayou for {G}
  Giant Growth (N) gives Grizzly Bears (N) +3/+3 until end of turn

Reproducer (G side; only green source is Bayou):
...
```
Expect in stdout: `Tap Bayou for {G}` and `Giant Growth (N) gives`.
...
CARD STATUS: WORKING — both intrinsic mana colours ({B}/{G}) produced and spendable.
