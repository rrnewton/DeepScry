...
updated_at: 2026-05-31T01:48:09.127503807+00:00
---
...
Set / deck: 1994 Old School playtest; deck 03_robots_jesseisbak (mtg-559); umbrella mtg-563
...
target is engine-auto-chosen rather than player-chosen (mtg-583), with rules-correct outcome.

Findings (2026-05-30_#2533(b052ce01), compat-wave16-robots):

[WORKING] Re-verified this pass for the robots deck sweep. The Abyss's upkeep
trigger destroys the active player's nonartifact creature each upkeep and
cannot be regenerated; the artifact Ornithopter is correctly spared.

Reproducer:

```sh
./target/release/mtg tui --start-state /tmp/abyss.pzl --p1=zero --p2=zero \
  --stop-on-choice=2 --seed 42 --verbosity 3
## (puzzle: p0 battlefield = The Abyss; Grizzly Bears; Ornithopter; lands)
```

Expected log evidence:

```
Trigger: The Abyss - At the beginning of each player's upkeep, destroy target
  nonartifact creature that player controls of their choice. It can't be regenerated.
Grizzly Bears (8) goes to graveyard
```

The single PARTIAL nuance (engine auto-selects the target rather than the
controller choosing) is rules-correct in outcome and tracked separately as
mtg-583. Per compatibility_tracking §3, the primary printed ability resolves
correctly with correct log output -> WORKING. Closing; mtg-583 carries the
player-choice follow-up.

CARD STATUS: WORKING — per-upkeep destroy of active player's nonartifact
creature, regen-proof; artifact creatures spared. (mtg-583 tracks the
player-choice-of-target refinement.)
