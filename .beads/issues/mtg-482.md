...
updated_at: 2026-05-29T14:30:31.667783792+00:00
---
...
Badlands is a NO-printed-mana-ability dual land (Types: Land Swamp Mountain).
Both colours come from intrinsic "{T}: Add {color}" abilities granted by the
two basic land subtypes per CR 305.6. The loader
(mtg-engine/src/loader/card.rs:500-565) already adds one mana ability per
basic land subtype, so NO engine change was needed — this is a
verification-to-WORKING.

- [WORKING] Parses as Land with subtypes Swamp + Mountain; no Effect::Unimplemented.
- [WORKING] Grants exactly two intrinsic {T} mana abilities: {B} and {R}.
- [WORKING] Mana from this card pays for spells: taps for {B} to cast Sengir
  Vampire ({3}{B}{B}) and taps for {R} to cast Lightning Bolt ({R}).

Expected log evidence:
  Tap Badlands for {B}
  Tap Badlands for {R}
  Lightning Bolt (N) deals 3 damage to Grizzly Bears

Reproducer (R side; only red source is Badlands):

```sh
./target/release/mtg tui --start-state test_puzzles/dual_lands_mana.pzl \
  --p1=fixed --p2=zero \
  --p1-fixed-inputs="cast Lightning Bolt;*;cast Giant Growth;*" \
  --stop-on-choice=8 --seed 42 --verbosity 3
```
Expect in stdout: `Tap Badlands for {R}` and `Lightning Bolt (N) deals 3 damage`.

Unit test: test_card_compat_original_dual_lands in mtg-engine/src/game/actions/tests/effects.rs
E2E test:  tests/dual_lands_mana_e2e.sh

CARD STATUS: WORKING — both intrinsic mana colours ({B}/{R}) produced and spendable.
